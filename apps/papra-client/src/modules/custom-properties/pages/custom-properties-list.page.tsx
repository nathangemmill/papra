...
const TYPE_CONFIG: Record<CustomPropertyType, { label: string; icon: string }> = {
  text: { label: 'Text', icon: 'i-tabler-text-size' },
  number: { label: 'Number', icon: 'i-tabler-123' },
  date: { label: 'Date', icon: 'i-tabler-calendar' },
  boolean: { label: 'Boolean', icon: 'i-tabler-toggle-left' },
  select: { label: 'Select', icon: 'i-tabler-list' },
  multi_select: { label: 'Multi-select', icon: 'i-tabler-list-check' },
};
...
export const DeleteCustomPropertyButton: Component<{ organizationId: string; propertyDefinitionId: string; propertyDefinitionName: string }> = (props) => {
  ...
  const deleteMutation = useMutation(() => ({
    ...
    onError: () => {
      createToast({
        message: 'Failed to delete custom property',
        type: 'error',
      });
    },
  }));
  ...
  const handleDelete = async () => {
    const isConfirmed = await confirm({
      ...
      message: `Are you sure you want to delete the custom property "${props.propertyDefinitionName}"? This action cannot be undone.`,
      confirmButton: {
        text: 'Delete',
        variant: 'destructive',
      },
...
export const CustomPropertiesPage: Component = () => {
  ...
  const table = createSolidTable({
    ...
    columns: [
        ...
        cell: (data) => {
          ...
          return (
            <div class="flex items-center gap-1.5 w-fit px-2 py-0.5 rounded bg-muted text-xs font-medium">
              <div class={`${config.icon} size-3.5 text-muted-foreground`} />
              {config.label}
            </div>
          );
        },
      },
      {
        ...
        cell: data => (
          <span class="text-wrap">
            {data.getValue<string | null>() ?? <span class="text-muted-foreground">No description</span>}
          </span>
        ),
      },
      {
      ...
      {
        id: 'actions',
        header: () => <span class="text-right">Actions</span>,
        cell: data => (
            ...
            <Show
              ...
              fallback={(
                <EmptyState
                  ...
                  icon="i-tabler-forms"
                  description="Custom properties let you add structured metadata to your documents, such as expiration dates, company names, or amounts."
                  cta={(
