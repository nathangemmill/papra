  ...
  describe('migrations', () => {
    ...
    test('regression test of the database state after running migrations, update the snapshot when the database state changes', async () => {
      ...
      expect(await serializeSchema({ db })).toMatchInlineSnapshot(`
        ...
        CREATE INDEX "custom_property_definitions_organization_id_display_order_index" ON "custom_property_definitions" ("organization_id", "display_order");
        ...
        CREATE INDEX "custom_property_select_options_definition_id_display_order_index" ON "custom_property_select_options" ("property_definition_id", "display_order");
        CREATE UNIQUE INDEX "custom_property_select_options_definition_id_value_unique" ON "custom_property_select_options" ("property_definition_id", "value");
        ...
        CREATE TABLE "custom_property_definitions" ( "id" text PRIMARY KEY NOT NULL, "created_at" integer NOT NULL, "updated_at" integer NOT NULL, "organization_id" text NOT NULL REFERENCES "organizations"("id") ON DELETE CASCADE ON UPDATE CASCADE, "name" text NOT NULL, "description" text, "type" text NOT NULL, "display_order" integer NOT NULL DEFAULT 0 );
        CREATE TABLE "custom_property_select_options" ( "id" text PRIMARY KEY NOT NULL, "created_at" integer NOT NULL, "updated_at" integer NOT NULL, "property_definition_id" text NOT NULL REFERENCES "custom_property_definitions"("id") ON DELETE CASCADE ON UPDATE CASCADE, "value" text NOT NULL, "display_order" integer NOT NULL DEFAULT 0 );
