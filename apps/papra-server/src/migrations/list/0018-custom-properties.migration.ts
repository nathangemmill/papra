...
export const customPropertiesMigration = {
  ...
  up: async ({ db }) => {
    await db.batch([
      ...
      db.run(sql`
        CREATE UNIQUE INDEX IF NOT EXISTS "custom_property_definitions_organization_id_normalized_name_unique"
        ON "custom_property_definitions" ("organization_id", "normalized_name")
      `),
      ...
      db.run(sql`
        CREATE UNIQUE INDEX IF NOT EXISTS "custom_property_select_options_definition_id_normalized_name_unique"
        ON "custom_property_select_options" ("property_definition_id", "normalized_name")
      `),
  ...
  down: async ({ db }) => {
    await db.batch([
      ...
      db.run(sql`DROP INDEX IF EXISTS "document_custom_property_values_document_id_index"`),
      db.run(sql`DROP INDEX IF EXISTS "document_custom_property_values_definition_text_index"`),
      db.run(sql`DROP INDEX IF EXISTS "document_custom_property_values_definition_number_index"`),
      db.run(sql`DROP INDEX IF EXISTS "document_custom_property_values_definition_date_index"`),
      db.run(sql`DROP INDEX IF EXISTS "document_custom_property_values_definition_boolean_index"`),
    ]);
  },
} satisfies Migration;
