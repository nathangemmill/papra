...
async function updateDocument({ documentId, organizationId, name, content, documentDate, notes, db }: { documentId: string; organizationId: string; name?: string; content?: string; documentDate?: Date | null; notes?: string | null; db: Database }) {
  const [document] = await db
    .update(documentsTable)
